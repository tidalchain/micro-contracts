...
contract FinalizeWithdrawalTest is L1WethBridgeTest {
    ...
    function test_RevertWhen_FinalisedAndUnsuccessfulProve() public {
        // finalised
        vm.mockCall(
            address(bridgeProxy.micro()),
            abi.encodeWithSelector(bridgeProxy.micro().isEthWithdrawalFinalized.selector),
            abi.encode(true)
        );
        // prove is unsuccessful
        vm.mockCall(
            address(bridgeProxy.micro()),
            abi.encodeWithSelector(bridgeProxy.micro().proveL2MessageInclusion.selector),
            abi.encode(false)
        );
        bytes memory message = abi.encodePacked(
            functionSignature, // function selector - 4 bytes
    ...
    function test_SuccessfulNotYetFinalized() public {
        ...
        vm.mockCall(
            address(bridgeProxy.micro()),
            abi.encodeWithSelector(bridgeProxy.micro().finalizeEthWithdrawal.selector),
            abi.encode(true)
        );
        uint256 amount = 10000;
