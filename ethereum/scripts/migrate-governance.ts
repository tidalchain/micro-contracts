...
import { getL1TxInfo } from "../../micro/src/utils";

import { UpgradeableBeaconFactory } from "../../micro/typechain/UpgradeableBeaconFactory";
...
const L2ERC20BridgeABI = JSON.parse(
  fs
    .readFileSync(
      "../micro/artifacts-zk/cache-zk/solpp-generated-contracts/bridge/L2ERC20Bridge.sol/L2ERC20Bridge.json"
    )
    .toString()
).abi;
...
async function main() {
  ...
  program
    ...
    .action(async (cmd) => {
      ...
      const microSetPendingGovernor = micro.interface.encodeFunctionData("setPendingGovernor", [
        governanceAddressFromEnv,
      ]);
      displayTx("micro Diamond Proxy migration calldata:", {
        data: microSetPendingGovernor,
        to: micro.address,
      });
      ...
      const calls = [
        {
          target: micro.address,
          value: 0,
          data: micro.interface.encodeFunctionData("acceptGovernor"),
        },
        {
