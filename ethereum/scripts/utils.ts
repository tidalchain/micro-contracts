...
export function readBatchBootloaderBytecode() {
  const bootloaderPath = path.join(process.env.MICRO_HOME as string, "etc/system-contracts/bootloader");
...
export function readSystemContractsBytecode(fileName: string) {
  const systemContractsPath = path.join(process.env.MICRO_HOME as string, "etc/system-contracts");
