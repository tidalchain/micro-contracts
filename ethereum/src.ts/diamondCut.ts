...
export async function getDeployedFacetCutsForRemove(wallet: Wallet, microAddress: string, updatedFaceNames: string[]) {
  const mainContract = IMicroFactory.connect(microAddress, wallet);
...
export async function getFacetCutsForUpgrade(
  ...
  const oldFacetCuts = await getDeployedFacetCutsForRemove(wallet, microAddress, namesOfFacetsToBeRemoved);
