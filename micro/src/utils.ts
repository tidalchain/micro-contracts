...
import { REQUIRED_L1_TO_L2_GAS_PER_PUBDATA_LIMIT, sleep } from "@zkamoeba/micro-web3/build/src/utils";
...
export async function awaitPriorityOps(
  ...
  microInterface: ethers.utils.Interface
) {
  const deployL2TxHashes = l1TxReceipt.logs
    .map((log) => microInterface.parseLog(log))
...
export async function getL1TxInfo(
  ...
  const micro = deployer.microContract(ethers.Wallet.createRandom().connect(provider));
  const l1Calldata = micro.interface.encodeFunctionData("requestL2Transaction", [
    to,
    0,
    l2Calldata,
  ...
  const neededValue = await micro.l2TransactionBaseCost(
    gasPrice,
    priorityTxMaxGasLimit,
    REQUIRED_L1_TO_L2_GAS_PER_PUBDATA_LIMIT
  );
